...
function _G.script_properties()
  ...
  obs.obs_properties_add_int(props, "triggerperiod", "Trigger Interval\n(in miliseconds)", 0, 100000, 100)

  local anychange = obs.obs_properties_add_bool(props, "anychange", "Trigger on Any Change?")
  obs.obs_property_set_modified_callback(anychange, checkboxes_update)

  local contents = obs.obs_properties_add_text(props, "contents", "Match Pattern", obs.OBS_TEXT_DEFAULT)
  obs.obs_property_set_long_description(contents, "Uses Lua pattern matching, see [shorturl.at/jltA8]\n\nThe provided default pattern is functional.\n\nNOTE: Whitespace characters are stripped from end of file before matching\n\nDo not edit if you don't know what this does.")

  local contentsmatch = obs.obs_properties_add_bool(props, "contentsmatch", "Display source(s) while content matches?")
  obs.obs_property_set_modified_callback(contentsmatch, checkboxes_update)

  obs.obs_properties_add_int(props, "duration", "Source Visibility\n(in miliseconds)", 1, 100000, 100)
  ...
  local scenechoice = obs.obs_properties_add_list(props, "scenechoice", "Only Scene(s)", obs.OBS_COMBO_TYPE_EDITABLE, obs.OBS_COMBO_FORMAT_STRING)
...
function _G.script_description()
  return "Simple script that checks if Firestone is running.\n\nMade by LÆVIS#2910"
end
...
function _G.script_defaults(settings)
  ...
  obs.obs_data_set_default_string(settings, "contents", "init OwAd")
